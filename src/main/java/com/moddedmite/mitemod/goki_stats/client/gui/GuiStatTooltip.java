...
public class GuiStatTooltip extends Gui {
    ...
    public void draw(int drawX, int drawY, int mouseButton) {
        ...
        if (level >= this.stat.getLimit())
            messageColorMap.put(I18n.getString("ui.max.name"), -16724737);
        else
            messageColorMap.put(I18n.getString("ui.cost.name") + this.stat.getCost(level) + "xp", -16724737); // Cost

        if (GuiScreen.isCtrlKeyDown())
            messageColorMap.put(I18n.getString("ui.return.name") + this.stat.getCost(level) * GokiConfig.globalModifiers.globalRevertFactor + "xp", -16724737); // Cost

        int revertLevel = DataHelper.getPlayerRevertStatLevel(player, stat);
        if (revertLevel > 0) {
            if (GokiConfig.globalModifiers.globalMaxRevertLevel != -1)
                messageColorMap.put(I18n.getStringParams("ui.reverted.name", new Object[]{revertLevel, String.valueOf(GokiConfig.globalModifiers.globalMaxRevertLevel)}), -16724737); // Reverted
            else
                messageColorMap.put(I18n.getStringParams("ui.reverted.name", new Object[]{revertLevel, I18n.getString("ui.infinite.name")}), -16724737); // Reverted
        }

        if (GuiScreen.isCtrlKeyDown()) { // Is player reverting?
            if (DataHelper.canPlayerRevertStat(player, stat))
                messageColorMap.put(I18n.getString("ui.revert.name"), 0xff0467ff);
            else
                messageColorMap.put(I18n.getString("ui.norevert.name"), 0xfffb1a00);
        } else
            messageColorMap.put(I18n.getString("ui.hover.name"), 0xff0467ff);
